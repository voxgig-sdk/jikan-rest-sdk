...
class ProducerEntity
  ...
  def make
    opts = @_entopts.dup
    ProducerEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = JikanRestHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Producer, Hash] the current Producer data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = JikanRestHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Producer fields)
  def match_get
  ...
  #
  # @param reqmatch [ProducerLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Producer.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Producer, Hash] the loaded Producer; raises JikanRestError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = JikanRestHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [ProducerListMatch, Hash, nil] match filter (any subset of
  #   Producer fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Producer>, Array] the matching Producer items; raises JikanRestError on failure
