...
class ExternalDirectTest < Minitest::Test
  def test_direct_list_external
    setup = external_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-external", setup[:live] ? "live" : "unit")
    if _should_skip
...
def external_direct_setup(mockres)
