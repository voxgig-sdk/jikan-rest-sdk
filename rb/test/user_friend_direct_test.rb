...
class UserFriendDirectTest < Minitest::Test
  def test_direct_list_user_friend
    setup = user_friend_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-user_friend", setup[:live] ? "live" : "unit")
    if _should_skip
...
def user_friend_direct_setup(mockres)
