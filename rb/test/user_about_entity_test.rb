...
class UserAboutEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.UserAbout(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.UserAbout(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set JIKAN_REST_TEST_USER_ABOUT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    user_about_ref01_list_result = user_about_ref01_ent.list(user_about_ref01_match, nil)
...
def user_about_basic_setup(extra)
  ...
  entid_env_raw = ENV["JIKAN_REST_TEST_USER_ABOUT_ENTID"]
