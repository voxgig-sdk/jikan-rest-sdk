...
class PeopleSearchEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.PeopleSearch(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.PeopleSearch(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set JIKAN_REST_TEST_PEOPLE_SEARCH_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def people_search_basic_setup(extra)
  ...
  entid_env_raw = ENV["JIKAN_REST_TEST_PEOPLE_SEARCH_ENTID"]
