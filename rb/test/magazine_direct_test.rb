...
class MagazineDirectTest < Minitest::Test
  def test_direct_list_magazine
    setup = magazine_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-magazine", setup[:live] ? "live" : "unit")
    if _should_skip
...
def magazine_direct_setup(mockres)
