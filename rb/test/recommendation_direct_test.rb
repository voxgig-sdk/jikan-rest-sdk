...
class RecommendationDirectTest < Minitest::Test
  def test_direct_list_recommendation
    setup = recommendation_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-recommendation", setup[:live] ? "live" : "unit")
    if _should_skip
...
def recommendation_direct_setup(mockres)
