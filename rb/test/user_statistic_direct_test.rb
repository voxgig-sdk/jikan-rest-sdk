...
class UserStatisticDirectTest < Minitest::Test
  def test_direct_load_user_statistic
    setup = user_statistic_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-user_statistic", setup[:live] ? "live" : "unit")
    if _should_skip
...
def user_statistic_direct_setup(mockres)
