...
class MangaDirectTest < Minitest::Test
  def test_direct_list_manga
    setup = manga_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-manga", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_manga
    setup = manga_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-manga", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def manga_direct_setup(mockres)
