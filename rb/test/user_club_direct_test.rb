...
class UserClubDirectTest < Minitest::Test
  def test_direct_list_user_club
    setup = user_club_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-user_club", setup[:live] ? "live" : "unit")
    if _should_skip
...
def user_club_direct_setup(mockres)
