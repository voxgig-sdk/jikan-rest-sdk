...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = JikanRestOmni.make_runner(TEST_JSON_FILE, JikanRestSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : JikanRestOmni.errify(err)
    end
