...
-- by passing it.
function JikanRestSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function JikanRestSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "JikanRestSDK: graphql: " .. msg
    res.graphql = errors
  end
