...
describe("ClubDirect", function()
  it("should direct-list-club", function()
    local setup = club_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-club", setup.live and "live" or "unit")
  ...
  it("should direct-load-club", function()
    local setup = club_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-club", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function club_direct_setup(mockres)
