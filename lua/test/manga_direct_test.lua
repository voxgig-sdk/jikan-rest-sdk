...
describe("MangaDirect", function()
  it("should direct-list-manga", function()
    local setup = manga_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-manga", setup.live and "live" or "unit")
  ...
  it("should direct-load-manga", function()
    local setup = manga_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-manga", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function manga_direct_setup(mockres)
