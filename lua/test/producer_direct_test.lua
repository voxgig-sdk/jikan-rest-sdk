...
describe("ProducerDirect", function()
  it("should direct-list-producer", function()
    local setup = producer_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-producer", setup.live and "live" or "unit")
  ...
  it("should direct-load-producer", function()
    local setup = producer_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-producer", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function producer_direct_setup(mockres)
