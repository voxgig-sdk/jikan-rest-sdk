...
local UserFriendEntity = {}
UserFriendEntity.__index = UserFriendEntity


function UserFriendEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, UserFriendEntity)
  self._name = "user_friend"
  self._client = client
...
function UserFriendEntity:get_name()
  return self._name
end


function UserFriendEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return UserFriendEntity.new(self._client, opts)
end
...
function UserFriendEntity:mark_deleted()
  self._deleted = true
end


function UserFriendEntity:deleted()
  return true == self._deleted
end


function UserFriendEntity:data_set(args)
  if args ~= nil then
...
function UserFriendEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function UserFriendEntity:match_set(args)
  if args ~= nil then
...
function UserFriendEntity:match_get()
...
function UserFriendEntity:stream(action, args, callopts)
...
function UserFriendEntity:_run_op(ctx, post_done)
  local utility = self._utility
