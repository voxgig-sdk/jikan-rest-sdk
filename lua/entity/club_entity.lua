...
local ClubEntity = {}
ClubEntity.__index = ClubEntity


function ClubEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ClubEntity)
  self._name = "club"
  self._client = client
...
function ClubEntity:get_name()
  return self._name
end


function ClubEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ClubEntity.new(self._client, opts)
end
...
function ClubEntity:mark_deleted()
  self._deleted = true
end


function ClubEntity:deleted()
  return true == self._deleted
end


function ClubEntity:data_set(args)
  if args ~= nil then
...
function ClubEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ClubEntity:match_set(args)
  if args ~= nil then
...
function ClubEntity:stream(action, args, callopts)
...
function ClubEntity:_run_op(ctx, post_done)
  local utility = self._utility
