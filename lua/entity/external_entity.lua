...
local ExternalEntity = {}
ExternalEntity.__index = ExternalEntity


function ExternalEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ExternalEntity)
  self._name = "external"
  self._client = client
...
function ExternalEntity:get_name()
  return self._name
end


function ExternalEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ExternalEntity.new(self._client, opts)
end
...
function ExternalEntity:mark_deleted()
  self._deleted = true
end


function ExternalEntity:deleted()
  return true == self._deleted
end


function ExternalEntity:data_set(args)
  if args ~= nil then
...
function ExternalEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ExternalEntity:match_set(args)
  if args ~= nil then
...
function ExternalEntity:match_get()
...
function ExternalEntity:stream(action, args, callopts)
...
function ExternalEntity:_run_op(ctx, post_done)
  local utility = self._utility
