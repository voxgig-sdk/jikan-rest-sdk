...
local WatchPromoEntity = {}
WatchPromoEntity.__index = WatchPromoEntity


function WatchPromoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WatchPromoEntity)
  self._name = "watch_promo"
  self._client = client
...
function WatchPromoEntity:get_name()
  return self._name
end


function WatchPromoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WatchPromoEntity.new(self._client, opts)
end
...
function WatchPromoEntity:mark_deleted()
  self._deleted = true
end


function WatchPromoEntity:deleted()
  return true == self._deleted
end


function WatchPromoEntity:data_set(args)
  if args ~= nil then
...
function WatchPromoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WatchPromoEntity:match_set(args)
  if args ~= nil then
...
function WatchPromoEntity:match_get()
...
function WatchPromoEntity:stream(action, args, callopts)
...
---@param reqmatch WatchPromoListMatch
---@param ctrl? table
---@return WatchPromo[]
---@return string? err
function WatchPromoEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function WatchPromoEntity:_run_op(ctx, post_done)
  local utility = self._utility
