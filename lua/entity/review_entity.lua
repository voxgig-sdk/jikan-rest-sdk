-- JikanRest SDK Review entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ReviewEntity = {}
ReviewEntity.__index = ReviewEntity


function ReviewEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReviewEntity)
  self._name = "review"
  self._client = client
...
function ReviewEntity:get_name()
  return self._name
end


function ReviewEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReviewEntity.new(self._client, opts)
end
...
function ReviewEntity:mark_deleted()
  self._deleted = true
end


function ReviewEntity:deleted()
  return true == self._deleted
end


function ReviewEntity:data_set(args)
  if args ~= nil then
...
function ReviewEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReviewEntity:match_set(args)
  if args ~= nil then
...
function ReviewEntity:match_get()
...
function ReviewEntity:stream(action, args, callopts)
...
---@param reqmatch ReviewLoadMatch
---@param ctrl? table
---@return Review
---@return string? err
function ReviewEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ReviewEntity:_run_op(ctx, post_done)
  local utility = self._utility
