...
local UserHistoryEntity = {}
UserHistoryEntity.__index = UserHistoryEntity


function UserHistoryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, UserHistoryEntity)
  self._name = "user_history"
  self._client = client
...
function UserHistoryEntity:get_name()
  return self._name
end


function UserHistoryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return UserHistoryEntity.new(self._client, opts)
end
...
function UserHistoryEntity:mark_deleted()
  self._deleted = true
end


function UserHistoryEntity:deleted()
  return true == self._deleted
end


function UserHistoryEntity:data_set(args)
  if args ~= nil then
...
function UserHistoryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function UserHistoryEntity:match_set(args)
  if args ~= nil then
...
function UserHistoryEntity:match_get()
...
function UserHistoryEntity:stream(action, args, callopts)
...
---@param reqmatch UserHistoryListMatch
---@param ctrl? table
---@return UserHistory[]
---@return string? err
function UserHistoryEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function UserHistoryEntity:_run_op(ctx, post_done)
  local utility = self._utility
