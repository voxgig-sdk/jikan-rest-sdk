...
local ProducerEntity = {}
ProducerEntity.__index = ProducerEntity


function ProducerEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProducerEntity)
  self._name = "producer"
  self._client = client
...
function ProducerEntity:get_name()
  return self._name
end


function ProducerEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProducerEntity.new(self._client, opts)
end
...
function ProducerEntity:mark_deleted()
  self._deleted = true
end


function ProducerEntity:deleted()
  return true == self._deleted
end


function ProducerEntity:data_set(args)
  if args ~= nil then
...
function ProducerEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProducerEntity:match_set(args)
  if args ~= nil then
...
function ProducerEntity:match_get()
...
function ProducerEntity:stream(action, args, callopts)
...
function ProducerEntity:_run_op(ctx, post_done)
  local utility = self._utility
