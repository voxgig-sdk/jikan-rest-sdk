...
local TopEntity = {}
TopEntity.__index = TopEntity


function TopEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TopEntity)
  self._name = "top"
  self._client = client
...
function TopEntity:get_name()
  return self._name
end


function TopEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TopEntity.new(self._client, opts)
end
...
function TopEntity:mark_deleted()
  self._deleted = true
end


function TopEntity:deleted()
  return true == self._deleted
end


function TopEntity:data_set(args)
  if args ~= nil then
...
function TopEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TopEntity:match_set(args)
  if args ~= nil then
...
function TopEntity:stream(action, args, callopts)
...
---@param reqmatch TopLoadMatch
---@param ctrl? table
---@return Top
---@return string? err
function TopEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function TopEntity:_run_op(ctx, post_done)
  local utility = self._utility
