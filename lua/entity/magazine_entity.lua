...
local MagazineEntity = {}
MagazineEntity.__index = MagazineEntity


function MagazineEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MagazineEntity)
  self._name = "magazine"
  self._client = client
...
function MagazineEntity:get_name()
  return self._name
end


function MagazineEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MagazineEntity.new(self._client, opts)
end
...
function MagazineEntity:mark_deleted()
  self._deleted = true
end


function MagazineEntity:deleted()
  return true == self._deleted
end


function MagazineEntity:data_set(args)
  if args ~= nil then
...
function MagazineEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MagazineEntity:match_set(args)
  if args ~= nil then
...
function MagazineEntity:match_get()
...
function MagazineEntity:stream(action, args, callopts)
...
---@param reqmatch MagazineListMatch
---@param ctrl? table
---@return Magazine[]
---@return string? err
function MagazineEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function MagazineEntity:_run_op(ctx, post_done)
  local utility = self._utility
