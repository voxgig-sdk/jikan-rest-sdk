...
local UserClubEntity = {}
UserClubEntity.__index = UserClubEntity


function UserClubEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, UserClubEntity)
  self._name = "user_club"
  self._client = client
...
function UserClubEntity:get_name()
  return self._name
end


function UserClubEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return UserClubEntity.new(self._client, opts)
end
...
function UserClubEntity:mark_deleted()
  self._deleted = true
end


function UserClubEntity:deleted()
  return true == self._deleted
end


function UserClubEntity:data_set(args)
  if args ~= nil then
...
function UserClubEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function UserClubEntity:match_set(args)
  if args ~= nil then
...
function UserClubEntity:match_get()
...
function UserClubEntity:stream(action, args, callopts)
...
---@param reqmatch UserClubListMatch
---@param ctrl? table
---@return UserClub[]
---@return string? err
function UserClubEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function UserClubEntity:_run_op(ctx, post_done)
  local utility = self._utility
