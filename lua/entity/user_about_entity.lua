...
local UserAboutEntity = {}
UserAboutEntity.__index = UserAboutEntity


function UserAboutEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, UserAboutEntity)
  self._name = "user_about"
  self._client = client
...
function UserAboutEntity:get_name()
  return self._name
end


function UserAboutEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return UserAboutEntity.new(self._client, opts)
end
...
function UserAboutEntity:mark_deleted()
  self._deleted = true
end


function UserAboutEntity:deleted()
  return true == self._deleted
end


function UserAboutEntity:data_set(args)
  if args ~= nil then
...
function UserAboutEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function UserAboutEntity:match_set(args)
  if args ~= nil then
...
function UserAboutEntity:match_get()
...
function UserAboutEntity:stream(action, args, callopts)
...
---@param reqmatch UserAboutListMatch
---@param ctrl? table
---@return UserAbout[]
---@return string? err
function UserAboutEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function UserAboutEntity:_run_op(ctx, post_done)
  local utility = self._utility
