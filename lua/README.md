...
The Lua SDK for the JikanRest API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Anime(nil):list(nil, nil)
...
local result, err = client:JikanRest(nil):load(
  { id = "test01" }, nil
)
