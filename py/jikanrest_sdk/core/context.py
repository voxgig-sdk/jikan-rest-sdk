...
from jikanrest_sdk.core.helpers import get_ctx_prop, to_map
...
class JikanRestContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, JikanRestSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, JikanRestResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, JikanRestResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return JikanRestError(code, msg, self)
