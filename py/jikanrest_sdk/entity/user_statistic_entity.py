...
class UserStatisticEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return UserStatisticEntity(self._client, opts)
    ...
    def data_get(self) -> UserStatistic:
    ...
    def match_get(self) -> UserStatistic:
    ...
    def load(self, reqmatch=None, ctrl=None) -> UserStatistic:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.UserStatistic().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
