...
class TestRecommendationDirect:

    def test_should_direct_list_recommendation(self):
        setup = _recommendation_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-recommendation", "live" if setup["live"] else "unit")
        if _skip:
...
def _recommendation_direct_setup(mockres):
