...
class TestWatchEpisodeEntity:

    def test_should_create_instance(self):
        testsdk = JikanRestSDK.test(None, None)
        ent = testsdk.WatchEpisode(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.WatchEpisode(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.WatchEpisode(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _watch_episode_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "watch_episode." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set JIKAN_REST_TEST_WATCH_EPISODE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        watch_episode_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.watch_episode")))
        watch_episode_ref01_data = None
        if len(watch_episode_ref01_data_raw) > 0:
...
def _watch_episode_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/watch_episode/WatchEpisodeTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "JIKAN_REST_TEST_WATCH_EPISODE_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("JIKAN_REST_TEST_WATCH_EPISODE_ENTID"))
