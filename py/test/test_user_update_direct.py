...
class TestUserUpdateDirect:

    def test_should_direct_load_user_update(self):
        setup = _user_update_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-user_update", "live" if setup["live"] else "unit")
        if _skip:
...
def _user_update_direct_setup(mockres):
