...
class TestWatchPromoDirect:

    def test_should_direct_list_watch_promo(self):
        setup = _watch_promo_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-watch_promo", "live" if setup["live"] else "unit")
        if _skip:
...
def _watch_promo_direct_setup(mockres):
