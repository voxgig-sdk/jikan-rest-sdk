...
class TestExternalEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.External(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.External(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set JIKAN_REST_TEST_EXTERNAL_ENTID JSON to run live")
        client = setup["client"]
...
def _external_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("JIKAN_REST_TEST_EXTERNAL_ENTID"))
