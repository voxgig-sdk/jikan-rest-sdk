...
class TestUserClubDirect:

    def test_should_direct_list_user_club(self):
        setup = _user_club_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-user_club", "live" if setup["live"] else "unit")
        if _skip:
...
def _user_club_direct_setup(mockres):
