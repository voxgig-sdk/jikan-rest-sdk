...
class TestUserFriendDirect:

    def test_should_direct_list_user_friend(self):
        setup = _user_friend_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-user_friend", "live" if setup["live"] else "unit")
        if _skip:
...
def _user_friend_direct_setup(mockres):
