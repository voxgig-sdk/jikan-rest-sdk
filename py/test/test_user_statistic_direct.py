...
class TestUserStatisticDirect:

    def test_should_direct_load_user_statistic(self):
        setup = _user_statistic_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-user_statistic", "live" if setup["live"] else "unit")
        if _skip:
...
def _user_statistic_direct_setup(mockres):
