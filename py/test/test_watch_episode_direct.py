...
class TestWatchEpisodeDirect:

    def test_should_direct_list_watch_episode(self):
        setup = _watch_episode_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-watch_episode", "live" if setup["live"] else "unit")
        if _skip:
...
def _watch_episode_direct_setup(mockres):
