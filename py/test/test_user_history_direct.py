...
class TestUserHistoryDirect:

    def test_should_direct_list_user_history(self):
        setup = _user_history_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-user_history", "live" if setup["live"] else "unit")
        if _skip:
...
def _user_history_direct_setup(mockres):
