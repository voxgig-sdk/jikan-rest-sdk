...
class TestExternalDirect:

    def test_should_direct_list_external(self):
        setup = _external_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-external", "live" if setup["live"] else "unit")
        if _skip:
...
def _external_direct_setup(mockres):
