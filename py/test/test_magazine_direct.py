...
class TestMagazineDirect:

    def test_should_direct_list_magazine(self):
        setup = _magazine_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-magazine", "live" if setup["live"] else "unit")
        if _skip:
...
def _magazine_direct_setup(mockres):
