...
class TestPeopleSearchDirect:

    def test_should_direct_list_people_search(self):
        setup = _people_search_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-people_search", "live" if setup["live"] else "unit")
        if _skip:
...
def _people_search_direct_setup(mockres):
