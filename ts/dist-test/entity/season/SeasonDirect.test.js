...
(0, node_test_1.describe)('SeasonDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-season', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-season', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["season01", "year01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.season ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.season = candidateId;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['JIKAN_REST_TEST_SEASON_ENTID'];
