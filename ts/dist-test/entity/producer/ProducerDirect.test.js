...
(0, node_test_1.describe)('ProducerDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.JikanRestSDK({
    ...
    (0, node_test_1.test)('direct-load-producer', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-producer', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-producer', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-producer', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.JIKAN_REST_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.JikanRestSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['JIKAN_REST_TEST_PRODUCER_ENTID'];
    ...
    const client = new __1.JikanRestSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=ProducerDirect.test.js.map