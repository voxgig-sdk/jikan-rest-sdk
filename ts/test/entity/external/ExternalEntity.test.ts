...
describe('ExternalEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'external.' + op, live)) return
    }
...
function basicSetup(extra?: any) {
  ...
  let idmap = transform(
    ['external01','external02','external03','users01','users02','users03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
  ...
  const idmapEnvVal = process.env['JIKAN_REST_TEST_EXTERNAL_ENTID']
