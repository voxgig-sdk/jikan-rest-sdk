...
describe('UserFriendEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'user_friend.' + op, live)) return
    }
    ...
    const user_friend_ref01_list = await user_friend_ref01_ent.list(user_friend_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  let idmap = transform(
    ['user_friend01','user_friend02','user_friend03','users01','users02','users03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
