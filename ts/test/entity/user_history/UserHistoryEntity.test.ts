...
describe('UserHistoryEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'user_history.' + op, live)) return
    }
    ...
    const user_history_ref01_list = await user_history_ref01_ent.list(user_history_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  let idmap = transform(
    ['user_history01','user_history02','user_history03','users01','users02','users03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
