...
describe('MagazineEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'magazine.' + op, live)) return
    }
    ...
    const magazine_ref01_list = await magazine_ref01_ent.list(magazine_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['JIKAN_REST_TEST_MAGAZINE_ENTID']
