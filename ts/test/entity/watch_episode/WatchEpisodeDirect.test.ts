...
describe('WatchEpisodeDirect', async () => {
  ...
  test('direct-list-watch_episode', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-watch_episode', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['JIKAN_REST_TEST_WATCH_EPISODE_ENTID']
