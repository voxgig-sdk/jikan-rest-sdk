...
describe('UserClubEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'user_club.' + op, live)) return
    }
    ...
    const user_club_ref01_list = await user_club_ref01_ent.list(user_club_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  let idmap = transform(
    ['user_club01','user_club02','user_club03','users01','users02','users03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
