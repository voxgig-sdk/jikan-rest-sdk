...
describe('ProducerEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'producer.' + op, live)) return
    }
    ...
    const producer_ref01_list = await producer_ref01_ent.list(producer_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['JIKAN_REST_TEST_PRODUCER_ENTID']
