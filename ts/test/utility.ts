...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.JIKANREST_TEST_LIVE ||
    'TRUE' === process.env.JIKANREST_TEST_OVERRIDE
  ) {
  ...
  m.JIKANREST_TEST_EXPLAIN = process.env.JIKANREST_TEST_EXPLAIN || m.JIKANREST_TEST_EXPLAIN
