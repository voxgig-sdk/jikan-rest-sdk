...
class JikanRestNetsimFeature extends JikanRestBaseFeature
{
    ...
    private function _simulate(JikanRestContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(JikanRestContext $ctx, array $applied): void
    {
        $client = $this->client;
