...
class ProducerEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = JikanRestSDK::test(null, null);
        $ent = $testsdk->Producer(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Producer(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Producer(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = producer_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "producer." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set JIKAN_REST_TEST_PRODUCER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // Bootstrap entity data from existing test data.
        $producer_ref01_data_raw = Vs::items(Helpers::to_map(
            Vs::getpath($setup["data"], "existing.producer")));
        $producer_ref01_data = null;
        if (count($producer_ref01_data_raw) > 0) {
            $producer_ref01_data = Helpers::to_map($producer_ref01_data_raw[0][1]);
        }
...
function producer_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/producer/ProducerTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["producer01", "producer02", "producer03"] as $k) {
    ...
    $entid_env_raw = getenv("JIKAN_REST_TEST_PRODUCER_ENTID");
