...
class UserEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set JIKANREST_TEST_USER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function user_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["user01", "user02", "user03", "users01", "users02", "users03"] as $k) {
    ...
    $entid_env_raw = getenv("JIKANREST_TEST_USER_ENTID");
