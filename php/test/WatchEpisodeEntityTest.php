...
class WatchEpisodeEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = JikanRestSDK::test(null, null);
        $ent = $testsdk->WatchEpisode(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->WatchEpisode(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->WatchEpisode(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = watch_episode_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "watch_episode." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set JIKAN_REST_TEST_WATCH_EPISODE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $watch_episode_ref01_data = null;
        if (count($watch_episode_ref01_data_raw) > 0) {
...
function watch_episode_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/watch_episode/WatchEpisodeTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["watch_episode01", "watch_episode02", "watch_episode03"] as $k) {
    ...
    $entid_env_raw = getenv("JIKAN_REST_TEST_WATCH_EPISODE_ENTID");
