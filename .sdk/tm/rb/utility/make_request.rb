...
module JikanRestUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = JikanRestResponse.new({})
    ...
    if fetch_err
      ...
      response = JikanRestResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = JikanRestResponse.new(fetched)
    else
