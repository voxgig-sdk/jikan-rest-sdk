...
class JikanRestTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            JikanRestTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = JikanRestTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = JikanRestTestRunner.getenv("PROJECTENV_TEST_LIVE")
        override = JikanRestTestRunner.getenv("PROJECTENV_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = JikanRestTestRunner.getenv(key)
        ...
        explain = JikanRestTestRunner.getenv("PROJECTENV_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if JikanRestTestRunner._test_control is not None:
            return JikanRestTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                JikanRestTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            JikanRestTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = JikanRestTestRunner.load_test_control()
...
def load_env_local():
    JikanRestTestRunner.load_env_local()


def env_override(m):
    return JikanRestTestRunner.env_override(m)


def entity_data(v):
    return JikanRestTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return JikanRestTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return JikanRestTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return JikanRestTestRunner.load_test_control()


def live_client_options():
    return JikanRestTestRunner.live_client_options()


def live_delay_ms():
    return JikanRestTestRunner.live_delay_ms()
