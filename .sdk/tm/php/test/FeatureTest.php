...
class FtCtrl extends JikanRestControl
{
...
class FtClient
{
    ...
    private JikanRestUtility $utility;

    public function __construct(JikanRestUtility $utility, array $options)
    {
    ...
    public function get_utility(): JikanRestUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (JikanRestContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public JikanRestContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (JikanRestContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = JikanRestConfig::make_feature($name);
    ...
    private static function buildUrl(JikanRestSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof JikanRestError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new JikanRestResponse($fetched);
        }
    ...
    private function fail(JikanRestContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (JikanRestContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
