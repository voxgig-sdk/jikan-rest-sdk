...
{
    private static function utility(): JikanRestUtility
    {
        return new JikanRestUtility();
    }
    ...
    private static function ctx(array $over = []): JikanRestContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new JikanRestContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new JikanRestOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof JikanRestError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new JikanRestOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = JikanRestMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new JikanRestOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = JikanRestMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new JikanRestOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = JikanRestMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new JikanRestSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = JikanRestMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new JikanRestResponse([]),
            'result' => new JikanRestResult([]),
        ]);
        [, $err] = JikanRestMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new JikanRestSpec([]),
            'response' => null,
            'result' => new JikanRestResult([]),
        ]);
        [, $err] = JikanRestMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new JikanRestSpec(['step' => 's']),
            'response' => new JikanRestResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new JikanRestResult([]),
        ]);
        [, $err] = JikanRestMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new JikanRestSpec(['step' => 's']),
            'response' => new JikanRestResponse(self::resp(200, ['v' => 1])),
            'result' => new JikanRestResult([]),
        ]);
        [, $err] = JikanRestMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new JikanRestSpec(['step' => 's']),
            'response' => new JikanRestResponse(self::resp(200, ['v' => 2])),
            'result' => new JikanRestResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        JikanRestMakeResponse::call($ctx);
    ...
    {
        $preset = new JikanRestResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new JikanRestSpec([]),
            'response' => new JikanRestResponse([]),
            'result' => new JikanRestResult([]),
        ...
        [$got, $err] = JikanRestMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new JikanRestResult([])]);
        [, $err] = JikanRestMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new JikanRestSpec([]), 'result' => null]);
        [, $err] = JikanRestMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new JikanRestSpec(['step' => 's']),
            'result' => new JikanRestResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new JikanRestOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = JikanRestMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new JikanRestSpec(['step' => 's']),
            'result' => new JikanRestResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new JikanRestOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = JikanRestMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new JikanRestResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new JikanRestSpec([]),
            'result' => new JikanRestResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = JikanRestMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = JikanRestMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new JikanRestError('boom', 'boom');
        $utility->fetcher = function (JikanRestContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new JikanRestSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = JikanRestMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (JikanRestContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = JikanRestMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(JikanRestResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (JikanRestContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new JikanRestSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = JikanRestMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new JikanRestResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new JikanRestSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = JikanRestMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = JikanRestMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = JikanRestMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new JikanRestResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, JikanRestDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new JikanRestResult(['ok' => false])]);
        $this->expectException(JikanRestError::class);
        JikanRestDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new JikanRestResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', JikanRestMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        JikanRestMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new JikanRestBaseFeature();
        $b = new JikanRestBaseFeature();
        JikanRestFeatureAdd::call($ctx, $a);
        JikanRestFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): JikanRestBaseFeature
    {
        $f = new JikanRestBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        JikanRestFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): JikanRestContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new JikanRestSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = JikanRestPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new JikanRestResponse(['headers' => null]),
            'result' => new JikanRestResult([]),
        ]);
        JikanRestResultHeaders::call($ctx);
