...
class JikanRestMakeRequest
{
    public static function call(JikanRestContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new JikanRestResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new JikanRestResponse($fetched);
        } else {
