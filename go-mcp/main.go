// Command JikanRest-mcp is an MCP server exposing the JikanRest
// SDK as MCP tools.
//
// Two tools are registered:
//
//	JikanRest_list  — list records of an entity
//	JikanRest_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: JIKAN_REST_APIKEY carries the API key and
	// JIKAN_REST_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("JIKAN_REST_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("JIKAN_REST_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewJikanRestSDK(opts)
