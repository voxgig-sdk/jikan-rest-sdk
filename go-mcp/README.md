...
[MCP](https://modelcontextprotocol.io) server exposing the JikanRest SDK as
two agent tools — `jikan-rest_list` and `jikan-rest_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/jikan-rest-mcp)
make build

# 2. Provide credentials via the environment
export JIKAN_REST_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user jikan-rest \
  -- /absolute/path/to/jikan-rest-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./jikan-rest-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// jikan-rest_list: first page of records
{ "entity": "anime" }
{ "entity": "anime", "query": { } }

// jikan-rest_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/jikan-rest-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user jikan-rest \
     -- "$PWD"/dist/*/jikan-rest-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `jikan-rest_list` and `jikan-rest_load` tools now appear
   in new sessions. Ask the agent to *"list anime using jikan-rest"*
   and it calls `jikan-rest_list` with `{"entity":"anime"}`.
...
```sh
export JIKAN_REST_APIKEY=sk_live_xxx            # API key
export JIKAN_REST_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `jikan-rest_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `jikan-rest_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `JIKAN_REST_APIKEY` | API key sent with every request. |
| `JIKAN_REST_BASE` | Optional override of the API base URL. |
...
```sh
./jikan-rest-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"jikan-rest_load","arguments":{"entity":"anime","query":{"id":1}}}}'
```
