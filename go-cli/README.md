...
boru-driven command-line client **and** interactive REPL for the JikanRest
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/jikan-rest-cli)
make build
...
./jikan-rest-cli load 1 anime            # {id:1} shorthand
./jikan-rest-cli load '{id:1}' anime       # explicit match map
./jikan-rest-cli list character

# 5. Override the API base URL for a single call
JIKAN_REST_BASE=https://api.example.com ./jikan-rest-cli list anime

# 6. No arguments -> interactive REPL
./jikan-rest-cli
jikan-rest> list anime
jikan-rest> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/jikan-rest-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/jikan-rest-cli list anime
   ```
...
```sh
./jikan-rest-cli load 1 anime          # scalar shorthand for {id:1}
./jikan-rest-cli load '{id:1}' anime     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `jikan-rest>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/jikan-rest-cli`. |
...
(`./jikan-rest-cli <expr>`) and interactively (the REPL), and expressions compose the
