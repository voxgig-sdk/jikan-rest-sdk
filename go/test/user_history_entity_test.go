...
func TestUserHistoryEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set JIKANREST_TEST_USER_HISTORY_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func user_historyBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"user_history01", "user_history02", "user_history03", "users01", "users02", "users03", "username01"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("JIKANREST_TEST_USER_HISTORY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["JIKANREST_TEST_USER_HISTORY_ENTID"])
