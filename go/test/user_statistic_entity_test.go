...
func TestUserStatisticEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		userStatisticRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.user_statistic")))
...
func user_statisticBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"user_statistic01", "user_statistic02", "user_statistic03", "user01", "user02", "user03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("JIKAN_REST_TEST_USER_STATISTIC_ENTID")
	...
	idmapResolved := core.ToMapAny(env["JIKAN_REST_TEST_USER_STATISTIC_ENTID"])
