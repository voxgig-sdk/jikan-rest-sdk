...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("JIKAN_REST_TEST_LIVE") == "TRUE" ||
		os.Getenv("JIKAN_REST_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("JIKAN_REST_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.JikanRestSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.JikanRestError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.JikanRestSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.JikanRestError{Msg: msg, Code: code}
}
