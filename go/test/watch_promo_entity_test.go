...
func TestWatchPromoEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.WatchPromo(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.WatchPromo(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		watchPromoRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.watch_promo")))
...
func watch_promoBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("JIKAN_REST_TEST_WATCH_PROMO_ENTID")
	...
	idmapResolved := core.ToMapAny(env["JIKAN_REST_TEST_WATCH_PROMO_ENTID"])
