...
func TestClubEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Club(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Club(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set JIKAN_REST_TEST_CLUB_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		clubRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.club")))
		...
		clubRef01DataDt0LoadResult := core.ToMapAny(entityData(clubRef01DataDt0Loaded))
		if clubRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if clubRef01DataDt0LoadResult["id"] != clubRef01Data["id"] {
...
func clubBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("JIKAN_REST_TEST_CLUB_ENTID")
	...
	idmapResolved := core.ToMapAny(env["JIKAN_REST_TEST_CLUB_ENTID"])
