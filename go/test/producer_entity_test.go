...
func TestProducerEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Producer(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Producer(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set JIKAN_REST_TEST_PRODUCER_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		producerRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.producer")))
...
func producerBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("JIKAN_REST_TEST_PRODUCER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["JIKAN_REST_TEST_PRODUCER_ENTID"])
