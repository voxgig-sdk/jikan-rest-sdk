...
func TestAnimeEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Anime(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Anime(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set JIKAN_REST_TEST_ANIME_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		animeRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.anime")))
...
func animeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("JIKAN_REST_TEST_ANIME_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"JIKAN_REST_TEST_ANIME_ENTID": idmap,
		"JIKAN_REST_TEST_LIVE":      "FALSE",
		"JIKAN_REST_TEST_EXPLAIN":   "FALSE",
	})

	idmapResolved := core.ToMapAny(env["JIKAN_REST_TEST_ANIME_ENTID"])
