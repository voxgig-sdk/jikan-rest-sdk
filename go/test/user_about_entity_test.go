...
func TestUserAboutEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.UserAbout(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.UserAbout(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		userAboutRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.user_about")))
...
func user_aboutBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"user_about01", "user_about02", "user_about03", "user01", "user02", "user03", "username01"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("JIKAN_REST_TEST_USER_ABOUT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["JIKAN_REST_TEST_USER_ABOUT_ENTID"])
