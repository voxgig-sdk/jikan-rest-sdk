...
import (
	...
	omni "github.com/voxgig-sdk/jikan-rest-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.JikanRestError); is {
				out := map[string]any{
					"name":    "JikanRestError",
...
func omniCtx(arg any, client *sdk.JikanRestSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.JikanRestSDK); is {
			client = live
