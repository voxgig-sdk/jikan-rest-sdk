...
func TestRandomEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set JIKAN_REST_TEST_RANDOM_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func randomBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("JIKAN_REST_TEST_RANDOM_ENTID")
	...
	idmapResolved := core.ToMapAny(env["JIKAN_REST_TEST_RANDOM_ENTID"])
